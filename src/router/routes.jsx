      ...
      json.forEach(async (resObject, index) => {
        try {
          await insert(dbObject, DB_TABLE, TABLE_FIELDS, [
            index,
            ...TABLE_FIELDS.slice(1).map(
              (field) => resObject[field]
            )
          ])
        } catch (error) {
          console.error(
            'Error while creating and/or interacting with databse. See the error below for more information'
          )
          console.error(error)
        }
      })
